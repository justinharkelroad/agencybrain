-- Phase 1: Fix find_or_create_contact RPC
--
-- Fixes:
-- 1. Add service role detection (auth.role() = 'service_role')
-- 2. Add has_agency_access check for non-service-role calls
-- 3. Add SET search_path for security
-- 4. Add ON CONFLICT for race conditions
-- 5. Implement ambiguity-safe phone matching (count first, then select)
-- 6. Function ACL hardening
...
BEGIN
  ...
  -- Access check: service role bypasses (edge functions), otherwise check has_agency_access
...
COMMENT ON FUNCTION find_or_create_contact IS
  'Finds existing contact by phone (primary) or household_key (secondary), or creates new. '
  'Returns contact UUID. Service role calls bypass access check (edge functions). '
  'Phone matching is ambiguity-safe: only matches if exactly one contact has the phone.';

-- Function ACL hardening: restrict who can execute
REVOKE ALL ON FUNCTION find_or_create_contact(uuid,text,text,text,text,text,text,text,text) FROM PUBLIC;
REVOKE ALL ON FUNCTION find_or_create_contact(uuid,text,text,text,text,text,text,text,text) FROM anon;

-- Grant to intended roles
GRANT EXECUTE ON FUNCTION find_or_create_contact(uuid,text,text,text,text,text,text,text,text) TO service_role;
GRANT EXECUTE ON FUNCTION find_or_create_contact(uuid,text,text,text,text,text,text,text,text) TO authenticated;
