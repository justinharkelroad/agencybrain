-- Fix: Include Manager role in dashboard queries
-- Managers have scorecard form access parity with Hybrid users,
-- so their submissions should appear in both Sales and Service tabs.
...
base AS (
  ...
  JOIN submissions s
   ...
   AND s.final IS TRUE
  -- Include the selected role, Hybrid (appears in both tabs), and Manager (has form parity with Hybrid)
  WHERE md.role::text IN (p_role, 'Hybrid', 'Manager')
