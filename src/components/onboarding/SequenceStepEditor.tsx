...
import type { ActionType } from "@/hooks/useOnboardingSequences";
...
export function SequenceStepEditor({
  ...
  const [dayNumber, setDayNumber] = React.useState(step?.day_number ?? 0);
  const [actionType, setActionType] = React.useState<ActionType>(step?.action_type ?? 'call');
  const [title, setTitle] = React.useState(step?.title ?? '');
  const [description, setDescription] = React.useState(step?.description ?? '');
  const [scriptTemplate, setScriptTemplate] = React.useState(step?.script_template ?? '');

  // Reset form when step changes
  React.useEffect(() => {
    if (open) {
      setDayNumber(step?.day_number ?? Math.max(0, ...existingDays, -1) + 1);
      setActionType(step?.action_type ?? 'call');
      setTitle(step?.title ?? '');
      setDescription(step?.description ?? '');
      setScriptTemplate(step?.script_template ?? '');
    }
