...
DO $$ BEGIN
  CREATE UNIQUE INDEX checklist_template_items_scope_label_uq
...
FOR ALL
USING (
  -- allow read/write if agency row and user has access
...
DROP TRIGGER IF EXISTS sync_mci_on_file_insert ON public.agency_files;
CREATE TRIGGER sync_mci_on_file_insert
...
VALUES
  (NULL, 'Agency Handbook', true, 10, true),
  (NULL, 'W-4', true, 20, true),
  (NULL, 'Accountability Policy', true, 30, true),
