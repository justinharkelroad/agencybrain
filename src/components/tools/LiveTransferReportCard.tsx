...
import { SaveLiveTransferReportButton } from '@/components/SaveLiveTransferReportButton';
...
interface LiveTransferReportCardProps {
...
export const LiveTransferReportCard = ({ inputs, derived, onClose, isReadOnly }: LiveTransferReportCardProps) => {
  ...
  const getFileName = (extension: string) => {
    const sourceName = inputs.liveTransferSource?.replace(/[^a-zA-Z0-9]/g, '_') || 'LiveTransfer';
    const date = new Date().toISOString().split('T')[0];
    return `AgencyBrain_Live_Transfer_${sourceName}_${date}.${extension}`;
  };
  ...
  const handleCopy = async () => {
    ...
    const lines = [
      `LIVE TRANSFER PERFORMANCE REPORT`,
      `============================`,
      `Transfer Source: ${inputs.liveTransferSource || 'Unknown'}`,
      ...
      `Cost per Transfer: ${formatCurrency(inputs.costPerTransfer)}`,
      `Projected Commission: ${formatCurrency(derived.totalComp)}`,
      ``,
      `TRANSFER FUNNEL`,
      `Total Transfers: ${derived.totalTransfers}`,
      ...
      `Cost per Closed HH: ${formatCurrency(costPerClosedHH)}`,
      `Cost per Item Sold: ${formatCurrency(costPerItem)}`,
    ];
    try {
  ...
  return (
    ...
    >
        ...
        <div className="flex items-start justify-between flex-wrap gap-4">
          <div>
            ...
            <h2 style={{ color: COLORS.textPrimary }} className="text-3xl font-bold mt-1">
              {inputs.liveTransferSource || 'Unknown Source'}
            </h2>
      ...
      >
        <div className="flex items-center gap-2 mb-4">
          <PhoneCall className="h-4 w-4" style={{ color: COLORS.textPrimary }} />
          <span style={{ color: COLORS.textPrimary }} className="text-sm font-semibold">TRANSFER FUNNEL</span>
        </div>
        <div className="grid gap-4 grid-cols-2 md:grid-cols-4">
          {[
            { value: derived.totalTransfers, label: 'Total Transfers' },
      ...
      >
        ...
        <div className="grid gap-4 grid-cols-2 lg:grid-cols-4">
          {[
            { value: inputs.costPerTransfer, label: 'Cost per Transfer', threshold: 35 },
            { value: derived.costPerQuotedHH, label: 'Cost per Quoted HH', threshold: 100 },
            { value: costPerClosedHH, label: 'Cost per Closed HH', threshold: 300 },
            { value: costPerItem, label: 'Cost per Item Sold', threshold: 200 },
          ].map((item, i) => (
            <div 
              key={i}
              style={{ backgroundColor: COLORS.bgCardAlt }}
      ...
      >
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p style={{ color: COLORS.textSecondary }} className="mt-1">
              {verdict.color === 'green' && (
                `${inputs.liveTransferSource || 'This source'} is performing well. Consider increasing transfer volume while monitoring quality.`
              )}
              {verdict.color === 'yellow' && (
                `${inputs.liveTransferSource || 'This source'} is breaking even. Review lead quality and improve quote-to-close conversion.`
              )}
              {verdict.color === 'red' && (
                `${inputs.liveTransferSource || 'This source'} is not profitable. Negotiate better per-transfer pricing or reduce volume.`
              )}
              {verdict.color === 'gray' && (
                `Add more data to see recommendations for ${inputs.liveTransferSource || 'this source'}.`
              )}
            </p>
          </div>
        </div>
    ...
    {!isReadOnly && (
      <div className="flex items-center justify-center gap-3 mt-4 flex-wrap">
        ...
        <SaveLiveTransferReportButton inputs={inputs} derived={derived} />
      </div>
    )}
    </>
  );
...
export default LiveTransferReportCard;
