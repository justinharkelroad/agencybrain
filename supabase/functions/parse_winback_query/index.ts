...
function buildSystemPrompt(teamMembers: { id: string; name: string }[]): string {
...
11. Today's date is ${new Date().toISOString().split('T')[0]}.`;
}
...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = buildSystemPrompt(teamMembers);
