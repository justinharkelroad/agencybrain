...
-- Skip the migration entirely when sales does not exist in the current environment.

DO $$
BEGIN
  IF to_regclass('public.sales') IS NULL THEN
    RAISE NOTICE 'Skipping % because public.sales does not exist.', '20260123142055_fc5dc794-bc06-4141-b04a-c609e8cf4a12';
    RETURN;
  END IF;

  EXECUTE 'ALTER TABLE public.sales ADD COLUMN IF NOT EXISTS contact_id UUID REFERENCES public.agency_contacts(id)';
  EXECUTE 'CREATE INDEX IF NOT EXISTS idx_sales_contact_id ON public.sales(contact_id)';
  EXECUTE '
