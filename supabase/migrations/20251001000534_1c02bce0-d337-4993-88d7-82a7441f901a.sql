-- CRITICAL FIX: Change flatten_quoted_household_details_enhanced to use snake_case 'quoted_details'
-- Bug: Function was looking for 'quotedDetails' (camelCase) but data is stored as 'quoted_details' (snake_case)
-- This prevented submissions like James Toney (ID: 19ebc10a-6906-41c2-b786-82a300fbc890) from being flattened
...
BEGIN
  ...
  -- FIXED: Process each quoted detail using snake_case 'quoted_details' NOT camelCase 'quotedDetails'
...
-- Backfill all recent submissions using the CORRECT snake_case field name
DO $$
DECLARE
  sub_record RECORD;
...
BEGIN
  RAISE NOTICE 'Starting backfill with CORRECT snake_case field name (quoted_details)...';
  
  -- FIXED: Process submissions that have 'quoted_details' (snake_case) NOT 'quotedDetails' (camelCase)
  FOR sub_record IN 
    ...
    WHERE s.submitted_at >= (NOW() - INTERVAL '30 days')
      AND s.final = true
      AND s.payload_json ? 'quoted_details'  -- FIXED: snake_case
