-- Add objection_id and objection_name to quoted_household_details
-- Allows structured objection data to flow through the submission pipeline
-- instead of being embedded as text in detailed_notes.

-- ============================================================
-- A) Add columns
-- ============================================================
ALTER TABLE public.quoted_household_details
  ADD COLUMN IF NOT EXISTS objection_id UUID REFERENCES public.lqs_objections(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS objection_name TEXT;

CREATE INDEX IF NOT EXISTS idx_qhd_objection_id
  ON public.quoted_household_details(objection_id)
  WHERE objection_id IS NOT NULL;

-- ============================================================
-- B) Update flatten_quoted_household_details_enhanced
--    Same signature (p_submission_id uuid) so CREATE OR REPLACE is safe.
-- ============================================================
...
BEGIN
  ...
  LOOP
    ...
    -- Extract objection_id (UUID from JSON)
    BEGIN
      v_objection_id := (v_detail ->> 'objection_id')::UUID;
    EXCEPTION WHEN OTHERS THEN
      v_objection_id := NULL;
    END;

    -- Validate objection_id FK exists to prevent INSERT failures
    -- (objection could be deleted between form load and flatten)
    IF v_objection_id IS NOT NULL THEN
      IF NOT EXISTS (SELECT 1 FROM lqs_objections WHERE id = v_objection_id) THEN
        v_objection_id := NULL;
      END IF;
    END IF;

    -- Extract objection_name (denormalized snapshot)
    v_objection_name := v_detail ->> 'objection_name';
    ...
    -- Extract premium and convert to cents (strip commas from formatted numbers)
    ...
    -- Insert into quoted_household_details (includes zip_code + objection)
...
-- ============================================================
-- C) Backfill existing records from lqs_households
--    Uses the raw_json stored in extras to find the source household.
...
BEGIN
  ...
  WHERE qhd.extras->'raw_json'->>'_lqs_household_id' = h.id::text
    AND qhd.objection_id IS NULL
    AND h.objection_id IS NOT NULL;

  GET DIAGNOSTICS v_updated = ROW_COUNT;
  RAISE NOTICE 'Backfilled objection data for % quoted_household_details rows', v_updated;
END $$;
