...
-- Fix column alias: change computed_stage back to current_stage for client compatibility
DROP FUNCTION IF EXISTS public.get_contacts_by_stage(uuid, text, text, integer, integer);

CREATE FUNCTION public.get_contacts_by_stage(
  p_agency_id uuid,
...
RETURNS TABLE(
  ...
  emails text[],
  household_key text,
  current_stage text,  -- Changed back to current_stage for client compatibility
...
BEGIN
  ...
  WITH contact_stages AS (
    SELECT 
      ...
      CASE
        ...
        -- Priority 4: Renewal (uses renewal_records table)
        WHEN EXISTS (
        ...
        ELSE 'unknown'
      END AS current_stage,  -- Aliased as current_stage
      ...
      (
        ...
        FROM (
          ...
          WHERE wh.contact_id = c.id
          UNION ALL
          SELECT ra.created_at FROM renewal_activities ra
      ...
      (
        ...
        FROM (
          ...
          WHERE wh.contact_id = c.id
          UNION ALL
          SELECT ra.created_at as dt, ra.activity_type FROM renewal_activities ra
          JOIN renewal_records rr ON rr.id = ra.renewal_record_id
          WHERE rr.contact_id = c.id
        ) activities
        ORDER BY dt DESC
      ...
      (
        ...
        WHERE tm.id = COALESCE(
          ...
          -- Then renewal
          (SELECT rr.assigned_team_member_id FROM renewal_records rr WHERE rr.contact_id = c.id AND rr.is_active = true LIMIT 1),
          -- Then LQS household
  ...
  WHERE 
    -- Stage filter
    (p_stage IS NULL OR cs.current_stage = p_stage)
