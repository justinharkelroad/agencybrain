DO $$
BEGIN
  IF to_regclass('public.sales_goals') IS NULL THEN
    RAISE NOTICE 'Skipping sales goal promo migration: public.sales_goals does not exist.';
    RETURN;
  END IF;
  ...
  DROP POLICY IF EXISTS "Users can view assignments for their agency goals" ON public.sales_goal_assignments;
  ...
  DROP POLICY IF EXISTS "Agency users can manage promo assignments" ON public.sales_goal_assignments;
