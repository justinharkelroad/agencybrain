-- Phase 2: Staff Portal RPC Functions for VOIP Integration
-- All functions are SECURITY DEFINER and validate team_member_id belongs to an agency
...
GRANT EXECUTE ON FUNCTION get_staff_call_metrics(UUID, DATE, DATE) TO authenticated;
GRANT EXECUTE ON FUNCTION get_agency_call_metrics(UUID, DATE, DATE) TO authenticated;
GRANT EXECUTE ON FUNCTION get_contact_activities(UUID, UUID, INTEGER, INTEGER) TO authenticated;
GRANT EXECUTE ON FUNCTION get_call_events(UUID, TIMESTAMPTZ, TIMESTAMPTZ, TEXT, INTEGER, INTEGER) TO authenticated;
GRANT EXECUTE ON FUNCTION get_agency_voip_status(UUID) TO authenticated;
