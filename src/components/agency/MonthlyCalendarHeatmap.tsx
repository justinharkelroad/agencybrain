...
export function MonthlyCalendarHeatmap({ 
  ...
  if (isLoading) {
    return (
      <Card className="p-4">
        {showHeader && (
          <div className="mb-4 text-center font-semibold text-foreground">
    ...
    return (
      <Card className="p-4">
        ...
        <div className="space-y-3 text-center">
          ...
          <button 
            onClick={() => refetch()} 
            className="px-3 py-2 border border-border rounded-md hover:bg-accent hover:text-accent-foreground transition-colors text-sm"
          >
            Retry
          </button>
        </div>
  ...
  return (
    <Card className="p-4">
      ...
      {showLegend && (
        <div className="mb-3 text-sm flex justify-center gap-4">
      ...
      <TooltipProvider delayDuration={150}>
        <div className="grid grid-cols-7 gap-1" role="grid" aria-label="Monthly pass fail calendar">
          {cells.map((d, idx) => {
            ...
            const cell = (
              <div
                className={`h-8 rounded flex items-center justify-center text-xs font-medium ${bg} ${textColor} ${isToday ? "outline outline-1 outline-foreground/50" : ""} transition-colors`}
              >
                {d.getDate()}
              </div>
            );
      ...
      {data && data.days.length === 0 && (
        <p className="mt-3 text-sm text-muted-foreground text-center">No submissions this month.</p>
      )}
    </Card>
  );
}
