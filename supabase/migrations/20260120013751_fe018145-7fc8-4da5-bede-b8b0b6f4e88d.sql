-- Update get_contacts_by_stage to support sorting parameters
...
DECLARE
  total BIGINT;
BEGIN
  -- Get total count first (respecting filters)
  WITH filtered_contacts AS (
    SELECT DISTINCT ac.id
    FROM agency_contacts ac
    LEFT JOIN lqs_households lh ON lh.agency_id = ac.agency_id AND lh.household_key = ac.household_key
    LEFT JOIN winback_households wh ON wh.agency_id = ac.agency_id AND wh.household_key = ac.household_key
    LEFT JOIN renewal_records rr ON rr.agency_id = ac.agency_id AND rr.household_key = ac.household_key
    LEFT JOIN cancel_audit_records car ON car.agency_id = ac.agency_id AND car.household_key = ac.household_key AND car.is_active = true
    WHERE ac.agency_id = p_agency_id
      ...
      AND (p_stage IS NULL OR p_stage = '' OR
        CASE
          WHEN car.id IS NOT NULL AND car.status NOT IN ('resolved', 'saved') THEN 'cancel_audit'
          WHEN wh.id IS NOT NULL AND wh.status NOT IN ('won', 'lost', 'excluded') THEN 'winback'
          WHEN rr.id IS NOT NULL AND rr.current_status NOT IN ('contacted', 'renewed', 'cancelled') THEN 'renewal'
          WHEN lh.id IS NOT NULL AND lh.status = 'sold' THEN 'customer'
          WHEN lh.id IS NOT NULL AND lh.status = 'quoted' THEN 'quoted'
          ELSE 'open_lead'
        END = p_stage
      )
  )
