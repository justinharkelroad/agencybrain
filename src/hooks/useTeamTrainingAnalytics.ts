...
export function useTeamTrainingAnalytics() {
  const { user } = useAuth();

  return useQuery<TeamTrainingAnalytics>({
    queryKey: ['team-training-analytics', user?.id],
    enabled: !!user?.id,
    queryFn: async () => {
      // 1. Get agency_id
      ...
      // 3. Fetch all data sources in parallel (now including Sales Experience)
      const [
        spProgressRes,
        spLessonsRes,
        agencyProgressRes,
        ...
        // Sales Experience: assignments (agency-level, no staff_user_id column)
        supabase
          .from('sales_experience_assignments')
          .select('id, status, start_date, end_date')
          .eq('agency_id', agencyId),

        // Sales Experience: staff progress
        supabase
      ...
      const salesExpProgress = salesExpProgressRes.data || [];
      ...
      const staffMembers: StaffTrainingMember[] = staffUsers.map((staff) => {
        ...
        const spLastActivity = staffSp
          ...
          .sort()
          .reverse()[0] || null;
        // Use actual video seconds if available, fallback to estimated minutes
        ...
        // --- Agency Training (reads from staff_lesson_progress) ---
        ...
        const agencyStarted = staffAgency.length; // any row = started
        ...
        const agencyLastActivity = staffAgency
          ...
          .sort()
          .reverse()[0] || null;
        // Use elapsed time where both timestamps exist, fallback to estimated
        ...
        // --- Sales Experience ---
        // Assignments are agency-level (no staff_user_id). Enrollment = agency has assignments + staff has progress.
        const salesAssignmentIds = new Set(salesExpAssignments.map((a) => a.id));
        const staffSalesProgress = salesExpProgress.filter(
          (p) => p.staff_user_id === staff.id && salesAssignmentIds.has(p.assignment_id)
        );
        const salesEnrolled = salesExpAssignments.length > 0 && staffSalesProgress.length > 0;
