...
BEGIN
  IF to_regclass('public.policy_types') IS NULL THEN
    RAISE NOTICE 'Skipping 20260203170000_link_policy_types_to_product_types: table public.policy_types does not exist.';
    RETURN;
  END IF;

  IF to_regclass('public.product_types') IS NULL THEN
    RAISE NOTICE 'Skipping 20260203170000_link_policy_types_to_product_types: table public.product_types does not exist.';
    RETURN;
  END IF;
  ...
  ON public.policy_types(product_type_id)
  ...
  COMMENT ON COLUMN public.policy_types.product_type_id IS
