...
import { CheckCircle2, AlertTriangle, Users, Home, FileText, Link2 } from 'lucide-react';
import { cn } from '@/lib/utils';
import type { QuoteUploadResult } from '@/types/lqs';

interface QuoteUploadResultsModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  results: QuoteUploadResult;
}

export function QuoteUploadResultsModal({ open, onOpenChange, results }: QuoteUploadResultsModalProps) {
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <StatCard
              icon={Home}
              label="Households"
              value={results.householdsCreated + results.householdsUpdated}
              subtext={`${results.householdsCreated} new, ${results.householdsUpdated} updated`}
              iconColor="text-blue-500"
            />
            <StatCard
              icon={FileText}
              label="Quotes"
              value={results.quotesCreated + results.quotesUpdated}
              subtext={`${results.quotesCreated} new, ${results.quotesUpdated} updated`}
            ...
            <StatCard
              icon={Link2}
              label="Sales Linked"
              value={results.salesLinked}
              subtext={results.salesNoMatch > 0 ? `${results.salesNoMatch} no match` : 'to households'}
