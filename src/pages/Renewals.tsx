    ...
    <div className="mx-auto px-6 max-w-[1800px] py-6 space-y-6">
      ...
      {selectedIds.size > 0 && (
        <div className="flex items-center gap-4 p-3 bg-muted rounded-lg">
          ...
          <DropdownMenu><DropdownMenuTrigger asChild><Button variant="outline" size="sm"><UserCheck className="h-4 w-4 mr-2" />Assign <ChevronDown className="h-4 w-4 ml-2" /></Button></DropdownMenuTrigger><DropdownMenuContent className="max-h-64 overflow-y-auto"><DropdownMenuItem onClick={() => { if (!context) return; bulkUpdate.mutate({ ids: Array.from(selectedIds), updates: { assigned_team_member_id: null }, displayName: context.displayName, userId: context.userId }, { onSuccess: () => setSelectedIds(new Set()) }); }}>Unassigned</DropdownMenuItem>{context?.staffTeamMemberId && (<DropdownMenuItem onClick={() => { if (!context) return; bulkUpdate.mutate({ ids: Array.from(selectedIds), updates: { assigned_team_member_id: context.staffTeamMemberId! }, displayName: context.displayName, userId: context.userId }, { onSuccess: () => setSelectedIds(new Set()) }); }}>Assign to Me</DropdownMenuItem>)}{teamMembers.filter((m) => m.id !== context?.staffTeamMemberId).map((m) => (<DropdownMenuItem key={m.id} onClick={() => { if (!context) return; bulkUpdate.mutate({ ids: Array.from(selectedIds), updates: { assigned_team_member_id: m.id }, displayName: context.displayName, userId: context.userId }, { onSuccess: () => setSelectedIds(new Set()) }); }}>{m.name}</DropdownMenuItem>))}</DropdownMenuContent></DropdownMenu>
