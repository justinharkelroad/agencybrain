...
BEGIN
  IF to_regclass('public.sale_items') IS NULL THEN
    RAISE NOTICE 'Skipping 20260203182000_rebackfill_sales_after_motorcycle_added: table public.sale_items does not exist.';
    RETURN;
  END IF;

  IF to_regclass('public.policy_types') IS NULL OR to_regclass('public.product_types') IS NULL THEN
    RAISE NOTICE 'Skipping 20260203182000_rebackfill_sales_after_motorcycle_added: missing policy_types or product_types.';
    RETURN;
  END IF;

  IF to_regclass('public.sale_policies') IS NULL OR to_regclass('public.sales') IS NULL THEN
    RAISE NOTICE 'Skipping 20260203182000_rebackfill_sales_after_motorcycle_added: table public.sale_policies or public.sales does not exist.';
    RETURN;
  END IF;
