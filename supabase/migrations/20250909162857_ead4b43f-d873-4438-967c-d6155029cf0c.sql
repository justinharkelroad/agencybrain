-- BLOCKER 3: Update upsert_metrics_from_submission to include KPI version fields
...
BEGIN
  ...
  -- FIXED: compute hits (>= target) and score (weight if >= target)
  hits := 0; 
  score := 0;
  ...
  -- upsert metrics_daily with KPI version fields
  ...
  VALUES (
    ...
    p_kpi_version_id, p_label_at_submit, coalesce(p_submitted_at, now()),
    now()
  )
