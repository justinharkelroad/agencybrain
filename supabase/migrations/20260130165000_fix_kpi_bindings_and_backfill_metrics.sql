-- Fix KPI bindings for all forms and backfill missing metrics_daily entries
-- This addresses forms that may have lost their KPI bindings during configuration changes

-- Step 1: Ensure all active KPIs have at least one active kpi_version entry
-- Some KPIs may have had all their versions invalidated during configuration changes
...
WHERE k.is_active = true
  AND NOT EXISTS (
    SELECT 1 FROM kpi_versions kv
    WHERE kv.kpi_id = k.id
      AND kv.valid_to IS NULL
  );

-- Step 2: Re-bind all active forms to ensure they have proper KPI bindings
-- This uses the existing bind_form_kpis function
DO $$
DECLARE
  form_record RECORD;
  kpi_count INT;
...
BEGIN
  FOR form_record IN
    SELECT ft.id, ft.name, ft.agency_id,
           COALESCE(jsonb_array_length(ft.schema_json->'kpis'), 0) as schema_kpi_count
  ...
  LOOP
    ...
    WHERE fkb.form_template_id = form_record.id
      AND kv.valid_to IS NULL;  -- Only count active bindings

    -- If missing bindings, rebind
    IF binding_count = 0 AND form_record.schema_kpi_count > 0 THEN
      RAISE NOTICE 'Rebinding form: % (id: %) - had 0 active bindings, schema has % KPIs',
        form_record.name, form_record.id, form_record.schema_kpi_count;
      PERFORM bind_form_kpis(form_record.id);
    END IF;
  END LOOP;
END $$;

-- Step 3: Backfill metrics_daily for any finalized submissions that are missing
-- This re-processes submissions that may have failed due to missing bindings
DO $$
DECLARE
  sub_record RECORD;
...
BEGIN
  ...
  FOR sub_record IN
    ...
    WHERE s.final = true
      ...
      AND NOT EXISTS (
        ...
        WHERE md.team_member_id = s.team_member_id
          AND md.date = COALESCE(s.work_date, s.submission_date)
          AND md.final_submission_id IS NOT NULL
      )
    ORDER BY s.submitted_at DESC
  LOOP
    BEGIN
      -- Attempt to process this submission
  ...
  RAISE NOTICE 'Backfilled % submissions into metrics_daily', processed_count;
END $$;

-- Step 4: Create or replace a helper function that more gracefully handles missing bindings
-- This is a safety improvement for future submissions
...
BEGIN
  ...
  IF v_kpi_version_id IS NULL THEN
    RAISE NOTICE 'No KPI binding found for form %, attempting auto-bind...', s.template_id;
  ...
  -- FALLBACK: If still no binding, use a default placeholder
  -- This allows metrics to be recorded even without KPI versioning
  IF v_kpi_version_id IS NULL THEN
    RAISE WARNING 'No KPI binding found for form % after auto-bind attempt. Using fallback mode.', s.template_id;
    -- Use a NULL version but continue processing - metrics are still valuable
