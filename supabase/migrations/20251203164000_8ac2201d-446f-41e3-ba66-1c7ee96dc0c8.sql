-- Fix: Move v_current_detail assignment BEFORE nested DECLARE block
-- The assignment must be in the FOR loop scope, not inside the nested DECLARE/BEGIN block
...
DECLARE
  ...
  v_payload_keys text[];
  v_current_detail jsonb;  -- Store current detail to preserve scope
BEGIN
  ...
  BEGIN
    ...
    LOOP
      -- ✅ FIXED: Assignment BEFORE the nested DECLARE block (in FOR loop scope)
      v_current_detail := v_quoted_detail.value;
      ...
      BEGIN
        -- Extract prospect name (v_current_detail is now already populated)
        ...
        LOOP
          v_field_value := v_current_detail->>v_field_key;
        ...
        -- Insert record with preserved timestamp
        INSERT INTO quoted_household_details (
          ...
          NULLIF((v_current_detail->>'items_quoted')::int, 0),
          NULLIF((v_current_detail->>'policies_quoted')::int, 0),
          NULLIF((v_current_detail->>'premium_potential_cents')::bigint, 0),
          jsonb_build_object(
            'original_data', v_current_detail,
          ...
          COALESCE(v_submission.submitted_at, v_submission.created_at, v_submission.submission_date::timestamp with time zone)
        );
