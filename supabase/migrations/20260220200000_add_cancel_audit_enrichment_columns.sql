-- Add enrichment columns to cancel_audit_records for richer AI search
-- These fields come from the Allstate Excel reports: City, State, Zip Code, Company Code, Premium Old($)
...
ALTER TABLE cancel_audit_records
  ...
  ADD COLUMN IF NOT EXISTS premium_old_cents BIGINT DEFAULT 0;

-- Recreate upsert_cancel_audit_record with 5 new parameters
-- IMPORTANT: Preserving ALL existing COALESCE patterns, contact-finding logic,
-- status transitions, and payment cleanup behavior from the previous version.
...
BEGIN
  ...
  IF v_existing_id IS NOT NULL THEN
    ...
    UPDATE cancel_audit_records SET
      ...
      -- New enrichment columns with COALESCE to preserve existing values
