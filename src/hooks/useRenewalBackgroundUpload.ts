...
async function processStaffUpload(
  ...
  try {
    ...
    const staffAutoPromotedMsg = data.autoPromotedCount > 0 ? `, ${data.autoPromotedCount} auto-renewed` : '';
    if (data.errorCount === 0) {
      toast({
        title: 'Renewal Upload Complete!',
        description: `${totalProcessed} records processed (${data.newCount} new, ${data.updatedCount} updated${staffDroppedMsg}${staffAutoPromotedMsg}) from ${filename}`,
      });
    } else {
      toast({
        title: 'Upload completed with issues',
        description: `${totalProcessed} succeeded, ${data.errorCount} failed${staffDroppedMsg}${staffAutoPromotedMsg} from ${filename}`,
        variant: 'destructive',
      });
    }
  } catch (error: any) {
...
async function processInBackground(
  ...
  try {
    ...
    const autoPromotedMsg = autoPromotedCount > 0 ? `, ${autoPromotedCount} auto-renewed` : '';
    if (errorCount === 0) {
      toast({
        title: 'Renewal Upload Complete!',
        description: `${totalProcessed} records processed (${newCount} new, ${updatedCount} updated${droppedMsg}${autoPromotedMsg}) from ${filename}`,
      });
    } else {
      toast({
        title: 'Upload completed with issues',
        description: `${totalProcessed} succeeded, ${errorCount} failed${droppedMsg}${autoPromotedMsg} from ${filename}`,
        variant: 'destructive',
      });
    }
