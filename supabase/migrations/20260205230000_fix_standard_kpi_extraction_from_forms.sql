-- Fix: Standard KPI values not extracted when form fields use custom_kpi_* keys
--
-- Bug: When an agency's form uses the form builder, KPI fields get auto-generated keys
-- like "custom_kpi_1738XXXXX_XXXXX". When those KPI fields are linked to STANDARD KPIs
-- (e.g., cross_sells_uncovered, talk_minutes, outbound_calls), the extraction fails:
--
--   1. Standard extraction (lines 136-151) looks for s.p->'cross_sells_uncovered' → NULL
--      because the payload key is "custom_kpi_XXXXX", not the standard column name.
--   2. Custom KPI extraction (line 177) skips non-custom_ slugs with CONTINUE.
--   3. Result: value never extracted → stored as 0 in metrics_daily.
--
-- Fix: Remove the custom_-only filter and handle BOTH cases:
--   - Custom slugs → store in custom_kpis JSONB (existing behavior)
--   - Standard slugs → update the corresponding standard column variable
--
-- Affected agency example: 086ee643-3212-4984-a5d9-249bec7d4c91 (Lisa Orona)
-- Service scorecard ring_metrics includes standard slugs (cross_sells_uncovered,
-- talk_minutes, outbound_calls) but form field keys are custom_kpi_* format.
...
-- Reprocess all recent final submissions to fix metrics
-- This will re-extract values using the corrected logic
DO $$
DECLARE
  sub_record RECORD;
  reprocessed INT := 0;
  failed INT := 0;
BEGIN
  RAISE NOTICE 'Reprocessing recent submissions to fix standard KPI extraction...';

  FOR sub_record IN
    SELECT s.id, s.work_date, s.submission_date, tm.name as member_name,
           ft.name as form_name, ft.agency_id
    FROM submissions s
  ...
  LOOP
    ...
    EXCEPTION WHEN OTHERS THEN
      RAISE WARNING 'Failed to reprocess submission % (%): %',
        sub_record.id, sub_record.member_name, SQLERRM;
      failed := failed + 1;
    END;
  END LOOP;

  RAISE NOTICE 'Reprocessing complete: % succeeded, % failed', reprocessed, failed;
END $$;
