-- Fix owner/admin sale edits on databases that already applied the canonical
-- upsert migration. Postgres does not support MIN(uuid) in the household lookup
-- path, so updates would fail before re-linking the sale.
