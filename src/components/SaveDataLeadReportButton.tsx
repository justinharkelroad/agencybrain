...
interface SaveDataLeadReportButtonProps {
  inputs: MarketingInputs;
  derived: MarketingDerived;
  disabled?: boolean;
}

export const SaveDataLeadReportButton = ({ inputs, derived, disabled }: SaveDataLeadReportButtonProps) => {
  const [isSaving, setIsSaving] = useState(false);
  const [saved, setSaved] = useState(false);

  const canSave = inputs.leadSource && inputs.spend > 0 && derived.totalLeads > 0;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const title = `${inputs.leadSource} - ${new Date().toLocaleDateString()}`;
      
      const { error } = await supabase.from('saved_reports').insert({
        agency_id: profile.agency_id,
        user_id: user.id,
        report_type: 'data_lead',
        title,
...
export default SaveDataLeadReportButton;
