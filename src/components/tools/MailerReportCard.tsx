...
import { SaveMailerReportButton } from '@/components/SaveMailerReportButton';
...
interface MailerReportCardProps {
  inputs: MailerInputs;
...
export const MailerReportCard = ({ inputs, derived, onClose, isReadOnly }: MailerReportCardProps) => {
  ...
  const handleCopy = async () => {
    const costPerInbound = derived.inboundCalls > 0 ? inputs.spend / derived.inboundCalls : null;
    const costPerClosedHH = derived.closedHH > 0 ? inputs.spend / derived.closedHH : null;
    const lines = [
      `MAILER PERFORMANCE REPORT`,
      `============================`,
      `Mail Source: ${inputs.mailSource || 'Unknown'}`,
      ...
      `Cost per Piece: ${formatCurrency(inputs.costPerPiece)}`,
      `Projected Commission: ${formatCurrency(derived.totalComp)}`,
      ``,
      `MAILER FUNNEL`,
      `Total Mailers Sent: ${derived.totalMailersSent}`,
      `Inbound Calls: ${derived.inboundCalls}`,
      ...
      `COST ANALYSIS`,
      `Cost per Piece: ${formatCurrency(inputs.costPerPiece)}`,
      `Cost per Inbound Call: ${formatCurrency(costPerInbound)}`,
  ...
  return (
    ...
    >
        ...
        <div className="flex items-start justify-between flex-wrap gap-4">
          <div>
            ...
            <h2 style={{ color: COLORS.textPrimary }} className="text-3xl font-bold mt-1">
              {inputs.mailSource || 'Unknown Source'}
            </h2>
      ...
      >
        <div className="flex items-center gap-2 mb-4">
          <Mail className="h-4 w-4" style={{ color: COLORS.textPrimary }} />
          <span style={{ color: COLORS.textPrimary }} className="text-sm font-semibold">MAILER FUNNEL</span>
        </div>
        <div className="grid gap-4 grid-cols-2 md:grid-cols-5">
          {[
            { value: derived.totalMailersSent, label: 'Mailers Sent' },
            { value: derived.inboundCalls, label: 'Inbound Calls' },
      ...
      >
        ...
        <div className="grid gap-4 grid-cols-2 lg:grid-cols-4">
          {[
            { value: inputs.costPerPiece, label: 'Cost per Piece', threshold: 1 },
            { value: costPerInbound, label: 'Cost per Inbound Call', threshold: 50 },
      ...
      >
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p style={{ color: COLORS.textSecondary }} className="mt-1">
              {verdict.color === 'green' && (
                `${inputs.mailSource || 'This campaign'} is performing well. Consider scaling volume while monitoring response rates.`
              )}
              {verdict.color === 'yellow' && (
                `${inputs.mailSource || 'This campaign'} is breaking even. Test different creative or targeting to improve response rates.`
              )}
              {verdict.color === 'red' && (
                `${inputs.mailSource || 'This campaign'} is not profitable. Reduce volume or negotiate better piece costs.`
              )}
              {verdict.color === 'gray' && (
                `Add more data to see recommendations for ${inputs.mailSource || 'this campaign'}.`
              )}
            </p>
          </div>
        </div>
    ...
    {!isReadOnly && (
      <div className="flex items-center justify-center gap-3 mt-4 flex-wrap">
        ...
        <SaveMailerReportButton inputs={inputs} derived={derived} />
      </div>
    )}
    </>
  );
...
export default MailerReportCard;
